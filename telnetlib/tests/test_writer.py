...
def test_writer_instantiation_safety():
    ...
    telnetlib.TelnetWriter(transport=None, protocol=None, client=True)
    with pytest.raises(TypeError):
        # must define at least server=True or client=True
        telnetlib.TelnetWriter(transport=None, protocol=None)
    with pytest.raises(TypeError):
        # but cannot define both!
        telnetlib.TelnetWriter(transport=None, protocol=None, server=True, client=True)


def test_repr():
    """Test writer.__repr__ for client and server viewpoint."""
    srv = telnetlib.TelnetWriter(transport=None, protocol=None, server=True)
    clt = telnetlib.TelnetWriter(transport=None, protocol=None, client=True)
    assert repr(srv) == (
...
def test_illegal_2byte_iac():
    ...
    writer = telnetlib.TelnetWriter(transport=None, protocol=None, server=True)
    from telnetlib.telopt import IAC, SGA
...
def test_legal_2byte_iac():
    ...
    from telnetlib.telopt import IAC, NOP
...
def test_sb_interrupted():
    ...
    from telnetlib.telopt import IAC, SB, TM, SE

    writer = telnetlib.TelnetWriter(
        transport=None,
        protocol=None,
        server=True,
    )
...
async def test_iac_do_twice_replies_once(bind_host, unused_tcp_port):
    ...
    from telnetlib.telopt import IAC, DO, WILL, ECHO
    ...
    await telnetlib.create_server(
        protocol_factory=telnetlib.BaseServer,
        host=bind_host,
        shell=shell,
...
async def test_iac_dont_dont(bind_host, unused_tcp_port):
    """WILL/WONT replied only once for repeated DO."""
    from telnetlib.telopt import IAC, DONT, ECHO
...
async def test_send_iac_dont_dont(bind_host, unused_tcp_port):
    ...
    from telnetlib.telopt import DONT, ECHO
    ...
    _, client_writer = await telnetlib.open_connection(
...
async def test_slc_simul(bind_host, unused_tcp_port):
    ...
    from telnetlib.telopt import IAC, DO, WILL, ECHO, SGA
    ...
    class SimulSLCServer(telnetlib.BaseServer):
        slc_callbacks = [
            getattr(telnetlib.slc, "SLC_" + key)
    ...
    for byte, waiter in callbacks.items():
        assert waiter.done(), telnetlib.slc.name_slc_command(byte)
...
async def test_unhandled_do_sends_wont(bind_host, unused_tcp_port):
    ...
    from telnetlib.telopt import IAC, DO, NOP, WONT
...
def test_bad_iac():
    ...
    from telnetlib.telopt import NOP
...
async def test_send_ga(bind_host, unused_tcp_port):
    ...
    from telnetlib.telopt import IAC, GA
...
async def test_not_send_ga(bind_host, unused_tcp_port):
    ...
    from telnetlib.telopt import IAC, DO, WILL, SGA
...
async def test_send_eor(bind_host, unused_tcp_port):
    ...
    from telnetlib.telopt import IAC, DO, WILL, CMD_EOR, EOR
